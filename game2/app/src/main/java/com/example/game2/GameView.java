    ...
    public boolean onTouchEvent(MotionEvent event){
        ...
        switch (action){
            case MotionEvent.ACTION_DOWN:
                ...
                if (poseI < 0 || poseI >= board.length || poseJ < 0 || poseJ >= board[0].length) {
                    return true; // Ignore out-of-bounds touches
                }
            ...
            case  MotionEvent.ACTION_MOVE:
                if(gamestate==Gamestate.nothing){
                    ...
                    if(move&& (deltaX > 30 || deltaY> 30)){
                        //check hpw many pixels our finger moved....
                        // if we moved our finger more than 30 pixels we start checking in which direction

                        move=false;
                        if(Math.abs(oldX-newX)>Math.abs(oldY-newY)){
                            // if rhe first X touch - the end of the X touch is bhgger than the
                            //first Y touch - the end of the touch Y so the directions is ont the X directiom
                            // now we will chekc whic is bigger thae oldC or the newX for know if itd left or righht
    ...
    public void draw(Canvas canvas) {
        ...
        for ( int i=0 ; i<10;i++){
            for(int j=0;j<9;j++){
                ...
                canvas.drawLine(j*cellWidth,drawY  , cellWidth*j , drawY+cellWidth*9,p);
